...
// 1 if new moon (± 36 hours)
// 2 if full moon (± 36 hours)
func IsMoonEvent(obs Observer) uint8 {
	...
	if math.Abs(float64(fullmoon.Sub(obs.Date)/time.Hour)) < 36 {
		return 2
	}
	if math.Abs(float64(newmoon.Sub(obs.Date)/time.Hour)) < 36 {
		return 1
	}
	return 0
}
