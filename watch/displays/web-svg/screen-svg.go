...
func WriteSVG(fd io.Writer) {
	...
	canvas.Circle(S/2, S/2, S/2-0.75*fontSize, "fill:white")
	...
	canvas.Rect(S/2-0.3125*fontSize, 0, 0.3125*fontSize, 0.125*fontSize)
	for i := 0; i < 12; i++ {
		canvas.Rotate(30)
		canvas.Rect(S/2-0.3125*fontSize, 0, 0.3125*fontSize, 0.125*fontSize)
	}
	...
	canvas.Rect(int(math.Floor(float64(width)*0.44)), 0, int(math.Floor(float64(width)*0.064)), int(math.Floor(0.5*fontSize)), "fill:blue")
	canvas.Gend()
	canvas.Gend()

	timeStr := fmt.Sprintf("%02d %02d", reg.Hour.Status(), reg.Minute.Status())
	canvas.Text(int(math.Floor(float64(width)*0.5)), int(math.Floor(float64(height)*0.58)), timeStr, "text-anchor:middle;font-family:Courier;font-weight:bold;font-size:6.5em;letter-spacing: -0.065em;fill:black")
	...
	canvas.Circle((S/2)-6, 0, 6, "fill:yellow")
	canvas.Gend()
	canvas.Circle(S/2, int(math.Floor(S/3)), int(math.Floor(0.8*fontSize)), "fill:yellow")
	sun_az_str := fmt.Sprintf("%000.0f", reg.SunAz.Deg())
	canvas.Text(S/2, int(math.Floor(S/2.88)), sun_az_str, "text-anchor:middle;font-family:courier;font-size:0.875em;fill:black")
	...
	canvas.Polygon([]int{
		int(math.Floor(float64(width) * 0.31)),
		int(math.Floor(float64(width) * 0.7)),
		int(math.Floor(float64(width) * 0.65)),
		int(math.Floor(float64(width) * 0.36)),
		int(math.Floor(float64(width) * 0.31)),
	}, []int{
		int(math.Floor(float64(height) * 0.71)),
		int(math.Floor(float64(height) * 0.71)),
		int(math.Floor(float64(height) * 0.91)),
		int(math.Floor(float64(height) * 0.91)),
		int(math.Floor(float64(height) * 0.85)),
